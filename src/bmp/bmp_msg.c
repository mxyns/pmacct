...
bmp_process_msg_peer_up(char **bmp_packet, u_int32_t *len, struct bmp_peer *bmpp, const ParsedBmp *netgauze_parsed) {
  ...
  netgauze_bgp_process_open(peer_up_open_tx.ok, &bgp_peer_loc);
  ...
  netgauze_bgp_process_open(peer_up_open_rx.ok, &bgp_peer_rem);
...
void bmp_process_msg_route_monitor(char **bmp_packet, u_int32_t *len, struct bmp_peer *bmpp,
  ...
  if (bdata_result.tag == Err_bmp_data__BmpParseError) {
    Log(LOG_INFO,
  ...
  if (bgp_result.tag == Err_ParsedBgpUpdate__ParseError) {
    return;
  }
