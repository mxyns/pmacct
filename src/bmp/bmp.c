    ...
    */
	...
	goto read_data;
      ...
      for (peer = NULL, peers_idx = 0; peers_idx < config.bmp_daemon_max_peers; peers_idx++) {
	    ...
	    bmpp = &bmp_peers[peers_idx];
            ...
            if (DEFAULT_PACKET_PROCESSOR.bmp_peer_init(bmpp, FUNC_TYPE_BMP)) {
	      peer = NULL;
	      bmpp = NULL;
	    }
	...
	if (ret > 0) {
      ...
      if (ret <= 0) {
        ...
        DEFAULT_PACKET_PROCESSOR.bmp_peer_close(bmpp, FUNC_TYPE_BMP);
        recalc_fds = TRUE;
        goto select_again;
      }
    }
    ...
    do_term = FALSE;
    DEFAULT_PACKET_PROCESSOR.bmp_process_packet(peer->buf.base, peer->msglen, bmpp, &do_term);
