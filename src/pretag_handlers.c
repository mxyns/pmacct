...
int PT_map_label_handler(char *filename, struct id_entry *e, char *value, struct plugin_requests *req, int acct_type)
{
  ...
  if (config.pretag_label_encode_as_map) {
    ...
    if (value_copy) {
      for (token = strtok_r(value_copy, DEFAULT_SEP, &saveptr1); token; token = strtok_r(NULL, DEFAULT_SEP, &saveptr1)) {
        map_key = token;
        map_value = strtok_r(token, PRETAG_LABEL_KV_SEP, &saveptr2);
      ...
      Log(LOG_WARNING, "WARN ( %s/%s ): [%s] PT_map_label_handler() failed strndup()\n", config.name, config.type, filename);
      return TRUE;
    }
  }
...
int PT_map_src_port_handler(char *filename, struct id_entry *e, char *value, struct plugin_requests *req, int acct_type)
{
  int x = 0;

  if (req->ptm_c.load_ptm_plugin == PLUGIN_ID_TEE) req->ptm_c.load_ptm_res = TRUE;

  e->key.src_port.neg = pt_check_neg(&value, &((struct id_table *) req->key_value_table)->flags);

  e->key.src_port.n = atoi(value);

  for (x = 0; e->func[x]; x++) {
    if (e->func_type[x] == PRETAG_SRC_PORT) {
      Log(LOG_WARNING, "WARN ( %s/%s ): [%s] Multiple 'src_port' clauses part of the same statement.\n", config.name, config.type, filename);
      return TRUE;
    }
  }
...
int pretag_src_port_handler(struct packet_ptrs *pptrs, void *unused, void *e)
{
  struct id_entry *entry = e;
  struct struct_header_v5 *hdr = (struct struct_header_v5 *) pptrs->f_header;
  struct template_cache_entry *tpl = (struct template_cache_entry *) pptrs->f_tpl;
  ...
  case 9:
    if (tpl->fld[NF9_L4_SRC_PORT].count) {
      OTPL_CP_LAST_M(&tmp16, NF9_L4_SRC_PORT, 2);
    }
    break;
  case 5:
    tmp16 = ((struct struct_export_v5 *) pptrs->f_data)->srcport;
    break;
  default:
    return TRUE; /* this field does not exist: condition is always true */
  }
  ...
  if (entry->key.src_port.n == port) return (FALSE | entry->key.src_port.neg);
...
int SF_pretag_src_port_handler(struct packet_ptrs *pptrs, void *unused, void *e)
{
  struct id_entry *entry = e;
  SFSample *sample = (SFSample *) pptrs->f_data;
...
int PT_map_index_entries_mpls_vpn_id_in_handler(struct id_table_index *idx, int idx_hdlr_no, pm_hash_serial_t *hash_serializer, void *src)
{
  struct id_entry *src_e = (struct id_entry *) src;

  if (!idx || !hash_serializer || !src_e) return TRUE;

  hash_serial_append(hash_serializer, (char *)&src_e->key.mpls_vpn_id_in.n, sizeof(u_int32_t), TRUE);

  return FALSE;
}

int PT_map_index_entries_mpls_vpn_id_out_handler(struct id_table_index *idx, int idx_hdlr_no, pm_hash_serial_t *hash_serializer, void *src)
{
  struct id_entry *src_e = (struct id_entry *) src;

  if (!idx || !hash_serializer || !src_e) return TRUE;

  hash_serial_append(hash_serializer, (char *)&src_e->key.mpls_vpn_id_out.n, sizeof(u_int32_t), TRUE);
...
int PT_map_index_entries_src_port_handler(struct id_table_index *idx, int idx_hdlr_no, pm_hash_serial_t *hash_serializer, void *src)
{
  struct id_entry *src_e = (struct id_entry *) src;
...
int PT_map_index_fdata_mpls_vpn_id_in_handler(struct id_table_index *idx, int idx_hdlr, int idx_netmask, struct id_entry *e, pm_hash_serial_t *hash_serializer, void *src)
{
  struct packet_ptrs *pptrs = (struct packet_ptrs *) src;
  struct struct_header_v5 *hdr = (struct struct_header_v5 *) pptrs->f_header;
  struct template_cache_entry *tpl = (struct template_cache_entry *) pptrs->f_tpl;
  ...
  if (config.acct_type == ACCT_NF) {
    ...
    case 9:
      if (tpl->fld[NF9_INGRESS_VRFID].count) {
        OTPL_CP_LAST_M(&tmp32, NF9_INGRESS_VRFID, 4);
        e->key.mpls_vpn_id_in.n = ntohl(tmp32);
      }
    }
  }

  hash_serial_append(hash_serializer, (char *)&e->key.mpls_vpn_id_in.n, sizeof(u_int32_t), FALSE);

  return FALSE;
}

int PT_map_index_fdata_mpls_vpn_id_out_handler(struct id_table_index *idx, int idx_hdlr, int idx_netmask, struct id_entry *e, pm_hash_serial_t *hash_serializer, void *src)
{   
  struct packet_ptrs *pptrs = (struct packet_ptrs *) src;
  struct struct_header_v5 *hdr = (struct struct_header_v5 *) pptrs->f_header;
  struct template_cache_entry *tpl = (struct template_cache_entry *) pptrs->f_tpl;
  ...
  if (config.acct_type == ACCT_NF) {
    ...
    case 9:
      if (tpl->fld[NF9_EGRESS_VRFID].count) {
        OTPL_CP_LAST_M(&tmp32, NF9_EGRESS_VRFID, 4);
        e->key.mpls_vpn_id_out.n = ntohl(tmp32);
      }
    }
  }

  hash_serial_append(hash_serializer, (char *)&e->key.mpls_vpn_id_out.n, sizeof(u_int32_t), FALSE);
...
int PT_map_index_fdata_src_port_handler(struct id_table_index *idx, int idx_hdlr, int idx_netmask, struct id_entry *e, pm_hash_serial_t *hash_serializer, void *src)
{
  struct packet_ptrs *pptrs = (struct packet_ptrs *) src;
  struct struct_header_v5 *hdr = (struct struct_header_v5 *) pptrs->f_header;
  struct template_cache_entry *tpl = (struct template_cache_entry *) pptrs->f_tpl;
  ...
  if (config.acct_type == ACCT_NF) {
    ...
    e->key.src_port.n = ntohs(tmp16);
  }
  else if (config.acct_type == ACCT_SF) {
    e->key.src_port.n = sample->dcd_sport;
  }
  else return TRUE;
