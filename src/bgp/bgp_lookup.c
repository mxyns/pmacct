...
void bgp_follow_nexthop_lookup(struct packet_ptrs *pptrs, int type)
{
  ...
  if (nh_peer) {
    modulo = bms->route_info_modulo(nh_peer, NULL, NULL, NULL, bms->table_per_peer_buckets);

    // XXX: to be optimized 
    if (bms->table_per_peer_hash == BGP_ASPATH_HASH_PATHID) modulo_max = bms->table_per_peer_buckets;
    else if (bms->table_per_peer_hash == BGP_ASPATH_HASH_MPLSVPNRD) modulo_max = bms->table_per_peer_buckets;
    else modulo_max = 1;
...
u_int32_t bgp_route_info_modulo_pathid(struct bgp_peer *peer, rd_t *rd, path_id_t *path_id, struct bgp_msg_extra_data *bmed, int per_peer_buckets)
{
...
u_int32_t bgp_route_info_modulo_mplsvpnrd(struct bgp_peer *peer, rd_t *rd, path_id_t *path_id, struct bgp_msg_extra_data *bmed, int per_peer_buckets)
{
  struct bgp_misc_structs *bms = bgp_select_misc_db(peer->type);
